							...
							<form action="" method="GET">
								<div id=""  class="tim-kiem-nang-cao">
									...
									<div class="khoanggia">
										...
										<div style="display: flex;align-items: center;justify-content: space-around; padding-bottom:1rem;">										
										Từ:<input id="slider_pr" type="number" name="price_1" min="0" max="30000000" style="width:4rem;">
										Tới:<input id="slider_pr" type="number" name="price_2" min="0" max="30000000" style="width:4rem;" >
											<span style="font-family: var(--font-family-sans-serif);"></span>
											đ
										</div>
									</div>
									<!--  -->
...
<script>
		 // thanh trượt giá
		 var sliderMin = document.getElementById("slider_pr_1");
var sliderMax = document.getElementById("slider_pr_2");
var outputMin = document.getElementById("min");
var outputMax = document.getElementById("max");



// Update the output values when the sliders are changed
sliderMin.oninput = function() {
    outputMin.innerHTML = formatPrice(this.value);
};

sliderMax.oninput = function() {
    outputMax.innerHTML = formatPrice(this.value);
};

function load_data() {
    var inputMin = document.getElementById("slider_pr_1");
    var inputMax = document.getElementById("slider_pr_2");

    // Assuming session variables 'pr_min' and 'pr_max' are set for the price range
    <?php
    if(isset($_SESSION['pr_min']) && isset($_SESSION['pr_max'])) {
    ?>
        inputMin.value = <?=$_SESSION['pr_min']?>;
        inputMax.value = <?=$_SESSION['pr_max']?>;
    <?php
    } else {
    ?>
        inputMin.value = 0;
        inputMax.value = 30000000; // Set default max value
    <?php
    }
    ?>
    outputMin.innerHTML = formatPrice(inputMin.value);
    outputMax.innerHTML = formatPrice(inputMax.value);
}
